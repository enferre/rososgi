...
bool JointMotionGenerator::calculateDesiredValues(double t,
				...
				(*delta_q_d)[i] = delta_q_[i]
						+ 0.5
								* (1.0 / std::pow(delta_t_2_sync_[i], 3.0)
										* (t - t_1_sync_[i]
												- 2.0 * delta_t_2_sync_[i]
												- t_d[i])
										* std::pow((t - t_1_sync_[i] - t_d[i]),
												3.0)
										+ (2.0 * t - 2.0 * t_1_sync_[i]
												- delta_t_2_sync_[i]
												- 2.0 * t_d[i]))
								* dq_max_sync_[i] * sign_delta_q[i];
			} else {
...
void JointMotionGenerator::calculateSynchronizedValues() {
	...
	for (size_t i = 0; i < 7; i++) {
		if (std::abs(delta_q_[i]) > kDeltaQMotionFinished) {
			if (std::abs(delta_q_[i])
					< (3.0 / 4.0
							* (std::pow(dq_max_[i], 2.0) / ddq_max_start_[i])
							+ 3.0 / 4.0
									* (std::pow(dq_max_[i], 2.0)
											/ ddq_max_goal_[i]))) {
	...
	for (size_t i = 0; i < 7; i++) {
		if (std::abs(delta_q_[i]) > kDeltaQMotionFinished) {
			...
			double c = std::abs(delta_q_[i]) * ddq_max_goal_[i]
					* ddq_max_start_[i];
			...
			q_1_[i] = (dq_max_sync_)[i] * sign_delta_q[i]
					* (0.5 * (t_1_sync_)[i]);
		}
	}
}
